# Generated by Django 3.1 on 2020-09-08 07:17

from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('umoney', '0014_auto_20200906_1420'),
    ]

    operations = [
        migrations.CreateModel(
            name='TopupCheckReq',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created', models.DateTimeField(auto_now_add=True)),
                ('comment', models.CharField(blank=True, default='', max_length=100)),
                ('message_type_id', models.CharField(blank=True, default='', max_length=4)),
                ('primary_bit_map', models.CharField(blank=True, default='', max_length=16)),
                ('processing_code', models.CharField(blank=True, default='', max_length=6)),
                ('transmission_datetime', models.CharField(blank=True, default='', max_length=10)),
                ('transaction_unique', models.CharField(blank=True, default='', max_length=12)),
                ('terminal_id', models.CharField(blank=True, default='', max_length=40)),
                ('request_data_len', models.CharField(blank=True, default='', max_length=3)),
                ('tran_type', models.CharField(blank=True, default='', max_length=2)),
                ('card_number', models.CharField(blank=True, default='', max_length=16)),
                ('card_algorithm_id', models.CharField(blank=True, default='', max_length=2)),
                ('card_keyset_v', models.CharField(blank=True, default='', max_length=2)),
                ('card_transaction_seq_number', models.CharField(blank=True, default='', max_length=10)),
                ('card_random_number', models.CharField(blank=True, default='', max_length=16)),
                ('topup_amount', models.CharField(blank=True, default='', max_length=10)),
                ('card_pre_balance', models.CharField(blank=True, default='', max_length=10)),
                ('card_post_balance', models.CharField(blank=True, default='', max_length=10)),
                ('sign3', models.CharField(blank=True, default='', max_length=8)),
                ('result_code', models.CharField(blank=True, default='9000', max_length=4)),
            ],
            options={
                'ordering': ['created'],
            },
        ),
        migrations.CreateModel(
            name='TopupCheckResp',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('created', models.DateTimeField(auto_now_add=True)),
                ('comment', models.CharField(blank=True, default='', max_length=100)),
                ('message_type_id', models.CharField(blank=True, default='', max_length=4)),
                ('primary_bit_map', models.CharField(blank=True, default='', max_length=16)),
                ('processing_code', models.CharField(blank=True, default='', max_length=6)),
                ('response_code', models.CharField(blank=True, default='', max_length=2)),
                ('transmission_datetime', models.CharField(blank=True, default='', max_length=10)),
                ('transaction_unique', models.CharField(blank=True, default='', max_length=12)),
                ('terminal_id', models.CharField(blank=True, default='', max_length=40)),
                ('result_message_len', models.CharField(blank=True, default='', max_length=3)),
                ('result_message_data', models.CharField(blank=True, default='', max_length=64)),
                ('deposit_balance', models.CharField(blank=True, default='', max_length=10)),
                ('sign3', models.CharField(blank=True, default='', max_length=8)),
            ],
            options={
                'ordering': ['created'],
            },
        ),
    ]
