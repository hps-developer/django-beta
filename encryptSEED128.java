...
// reorder for little endian	  
	...
	public static void main(String[] args)
	{
		...
		byte randomValue[] = stringToBytesASCII(args[0].substring(0,16));
		byte decryptedWKey[] = stringToBytesASCII(args[0].substring(16, 32));
		byte pbWorkingKey[] = parseHexBinary(args[0].substring(32));
		byte pbDataX[] = new byte[16];
		int pdwRoundKey[]  = new int[32];
		for (int i=0; i<16; i++){
			pbDataX[i] = (byte) (randomValue[i]^decryptedWKey[i]) ;
		}
		SeedRoundKey(pdwRoundKey, pbWorkingKey);
		SeedEncrypt(pbDataX, pdwRoundKey, tBuffer);	
			
		//SeedDecrypt(tBuffer, pdwRoundKey, sBuffer);	
	    // for (int i=0; i<16; i++)	{
		// 	if(Integer.toHexString(0xff&pbDataX[i]).toUpperCase().length() > 1)
		// 		System.out.print(Integer.toHexString(0xff&pbDataX[i]).toUpperCase()) ;
		// 	else System.out.print("0" + Integer.toHexString(0xff&pbDataX[i]).toUpperCase());
		// }
		// System.out.println("");
	    // for (int i=0; i<16; i++){
		// 	if(Integer.toHexString(0xff&tBuffer[i]).toUpperCase().length() > 1)
		// 		System.out.print(Integer.toHexString(0xff&tBuffer[i]).toUpperCase()) ;
		// 	else System.out.print("0" + Integer.toHexString(0xff&tBuffer[i]).toUpperCase());
		// }
		// System.out.println("");
	    // for (int i=0; i<16; i++)	{
		// 	if(Integer.toHexString(0xff&sBuffer[i]).toUpperCase().length() > 1)
		// 		System.out.print(Integer.toHexString(0xff&sBuffer[i]).toUpperCase()) ;
		// 	else System.out.print("0" + Integer.toHexString(0xff&sBuffer[i]).toUpperCase());
		// }
		// System.out.println("");
	    for (int i=0; i<16; i++){
			if(Integer.toHexString(0xff&(tBuffer[i])).toUpperCase().length() > 1)
